...
include '../connect.php';
// $log_id=$_POST['log_id'];
$sql=mysqli_query($con,"SELECT * from goods_movement  order by gm_id desc");
$list=array();
if($sql->num_rows>0)
{
    while($row=mysqli_fetch_assoc($sql))
    {
        ...
        $myarray['goodsType']=$row['goodsType'];
        $myarray['weightLeft']=$row['weightLeft'];
        $myarray['vehModel']=$row['vehModel'];
