...
class _gm3State extends State<gm3> {
  ...
  var goodsType = TextEditingController();
  var weightLeft = TextEditingController();
  var vehModel = TextEditingController();
  ...
  Future<void> senddata() async {
    ...
    var data = {
      ...
      'weightLeft': weightLeft.text,
      'vehModel': vehModel.text,
      'date': datectrl.text,
      'time': timectrl.text,
      'log_id': log_id
    };
    var response =
        await post(Uri.parse("${con.url}goods_movement/goods.php"), body: data);
    print(response.body);
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Goods movement added')));
            ...
            child: Column(
              children: [
                ...
                Container(
                    height: 200,
                    child: Image(
                      image: AssetImage('assets/login/gd3.png'),
                      fit: BoxFit.contain,
                    )),
                SizedBox(
                  height: 20,
                ...
                Padding(
                  ...
                  child: TextFormField(
                    ...
                    controller: weightLeft,
                    style: TextStyle(color: Colors.black),
                    decoration: InputDecoration(
                        filled: true,
                        fillColor: Color(0xffDCDADA),
                        hintText: 'Available weight left for goods (in kg)',
                        border: OutlineInputBorder(
                            borderSide: BorderSide.none,
                            borderRadius: BorderRadius.circular(20))),
                  ),
