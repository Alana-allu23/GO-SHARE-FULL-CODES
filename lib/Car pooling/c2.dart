                  ...
                  builder: (context, snapshot) {
                    ...
                    return flag == 0
                        ...
                        : ListView.builder(
                            ...
                            shrinkWrap: true,
                            itemCount:
                                snapshot.data?.length ?? 0, // Add null check
                            itemBuilder: (context, index) {
                              var data = snapshot.data?[index];
                              if (data == null)
                                return SizedBox.shrink(); // Add null check
                              return InkWell(
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => offerpool(
                                        ...
                                        pool_id: data[
                                            'pool_id'], // Add null check for status
                                      ),
                                    ),
                                  );
                                },
                                child: ListTile(
                                  ...
                                  trailing: Column(
                                    children: [
                                      ...
                                      Text('${data['date']}'),
                                      Text(
                                          '${data['status'] ?? 'N/A'}'), // Display 'N/A' if status is not available
                                    ],
                                  ),
                                ),
                              );
